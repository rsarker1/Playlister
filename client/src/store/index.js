...
function GlobalStoreContextProvider(props) {
    ...
    // store.changeListName = function(id, newName) {
    //     async function asyncUpdateList(id, list) {
    //         console.log(list);
    //         console.log(id);
    ...
    //     }
    //     if(index != null) 
    //         asyncUpdateList(id, store.idNamePairs[index]);
    // }
    ...
    store.createNewList = async function () {
        ...
        const response = await api.createPlaylist(newListName, [], auth.user.email, auth.user.userName, auth.user.firstName, auth.user.lastName);
    ...
    store.addLike = function(id, userName) {
        ...
        if(index != null) 
            store.updateListViaFrontEnd(id, store.idNamePairs[index]);


        // async function asyncAddLike(id, userName) {
        //     let response = await api.getPlaylistById(id);
        //     if (response.data.success) {
        //         let playlist = response.data.playlist;
        ...
        //     }
        // }
        // asyncAddLike(id, userName);
    }
    store.addDislike = function(id, userName) {
        // async function asyncDisLike(id, userName) {
        //     let response = await api.getPlaylistById(id);
        //     if (response.data.success) {
        //         let playlist = response.data.playlist;
        //         if(playlist.dislikes.indexOf(userName) > -1) 
        //             return;
        //         if(playlist.likes.indexOf(userName) > -1)
        //             playlist.likes.splice(playlist.likes.indexOf(userName), 1);
        ...
        for (let i = 0; i < store.idNamePairs.length; i++) {
            if(store.idNamePairs[i]._id === id) {
                if(store.idNamePairs[i].dislikes.indexOf(userName) > -1) 
                    break;
                if(store.idNamePairs[i].likes.indexOf(userName) > -1)
                    store.idNamePairs[i].likes.splice(store.idNamePairs[i].likes.indexOf(userName), 1);
                store.idNamePairs[i].dislikes.push(userName);
                index = i;
                break;
            }
        }
    ...
    store.updateListViaFrontEnd = function(id, list) {
    ...
    store.loadPPByListname = function(name) {
        async function asyncLoadPPByListName() {
        ...
        asyncLoadPPByListName();
    }
    store.loadPPByUsername = function(userName) {
        async function asyncLoadPPByUsername() {
            const response = await api.getPPPairsByUsername(userName);
            if(response.data.success) {
                let pairsArray = response.data.idNamePairs;
                console.log(pairsArray);
                storeReducer({
    ...
    store.searchPlaylists = function(inputString) {
        ...
        if(store.currentState === CurrentState.USERS) 
            store.loadPPByUsername(inputString);
    }
    store.increaseListens = function(id) {
        let index = null;
        for(let i = 0; i < store.idNamePairs.length; i++) {
            if(store.idNamePairs[i]._id === id) {
                index = i;
        ...
        //     if(index != null) {
        //         const response = await api.updatePlaylistById(id, store.idNamePairs[index]);
        //     }
        // }
        // asyncWait();
        if(index != null) 
        ...
        // async function asyncIncreaseListens(id) {
        //     let response = await api.getPlaylistById(id);
        //     if (response.data.success) {
        //         let playlist = response.data.playlist;
        //         playlist.listens++;
